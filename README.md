...
- [\[Лекция 3\] Временные ряды](#лекция-3-временные-ряды)
  ...
  - [Тренд и сезонность](#тренд-и-сезонность)
  - [Стационарность](#стационарность)
  - [Как предсказывать](#как-предсказывать)
- [\[Лекция 4\] Ранжирование](#лекция-4-ранжирование)
  - [Что это?](#что-это)
...
Пытаемся предсказывать поведение временного ряда (курс валюты / потребление электричества / спрос на товар / ...)

Временной ряд можно разбить на несколько составляющих 
* **Тренд** (Куда вообще стремится временной ряд. Обычно описывается несложной монотонной функцией)
* **Сезонность** (Их может быть несколько: суточная сезонность, полугодовалая и т.д. Обычно задается синусоидой)
* **Остаток** (То, что остается после вычитания из ВР тренда и сезонностей. Обычно похож на шум)

Пример разбиения ВР на тренд, сезонности и остаток:
<p align="center">
<img src="./src/img/03_lect/trend_seasonal_remainder.png"  style="width: 50%">
</p>

Тренд и сезонность могут быть аддиктивными или мультипликативными
<p align="center">
<img src="./src/img/03_lect/trend_seasonal_add_mult.png"  style="width: 80%">
</p>

Для разложения ВР на тренд и сезонности есть STL разложение (на лекции про него практически ничего не рассказали, но можно прочитать здесь: https://education.yandex.ru/handbook/ml/article/vremennye-ryady)

## Стационарность

Временной ряд называется **"строго стационарным"**, если совместное распределение любой группы точек ряда не зависит от того, "когда" мы их взяли.Проверить строгую стационарность на практике очень сложно, поэтому используют ослабленное определение.

Временной ряд называется **"слабо стационарным"**, если:
* У ряда конечное мат. ожидание и конечная дисперсия
* Среднее значение и дисперсия постоянны во времени.
  * Среднее постоянно: Нет ни роста, ни падения, ни сезонных "горбов". Если нарисовать линию тренда, она будет горизонтальной
  * Дисперсия постоянна: Разброс данных вокруг среднего не меняется. Нет таких периодов, когда значения резко "разлетаются" или, наоборот, почти не колеблются
* Ковариация зависит только от "расстояния" во времени (лага). Например, связь между сегодняшним и вчерашним значением должна быть такой же, как связь между значением 100 дней назад и 99 дней наза. Эта связь не должна меняться со временем

<p align="center">
<img src="./src/img/03_lect/stationary_ts.png"  style="width: 50%">
</p>

Чтобы проверить, стационарный ли ряд, можно сделать "какой-то" тест, технические детали которого нам на лекции не рассказали, подробнее расскажут на курсе про временные ряды...

## Как предсказывать 

Возможные способы препроцессинга ВР:
* Логарифмизация / преобразование Box-Cox
* Удаляем сезонность
* Удаляем тренд
* Разложение Фурье
...
Способы предсказывания временного ряда:

<p align="center">
<img src="./src/img/03_lect/timeline.png"  style="width: 80%">
</p>

Какие фичи использовать для обучения:
* Календарные фичи
  * День недели, месяц, год, ...
* Lag и разностные фичи
  * Значение за предыдущий день или год назад
  * Разность между текущим значением и вчерашним / прошлогодним
* Статистические
  * Среднее, медиана, квантиль по группам (например, по дню недели)
  * Скользящее среднее и дисперсия (например, за 7 или 30 дней)
* Фичи из статистических моделей
  * компоненты моделей ARIMA или ETS

Данные из временного ряда можно записать в виде "длинной" или "широкой" таблицы. Чаще всего данные записывают в "длинном" виде: каждая строчка соответствует отдельной отметке времени. Но модели удобнее обучать на "широких" таблицах.

Как перевести таблицу из длинного формата в широкий:

<p align="center">
<img src="./src/img/03_lect/long_wide_dataset.png"  style="width: 80%">
</p>

Вводим параметры STEP, L и H. Берем L+H строк в исходном длинном датасете с шагом STEP и делаем широкий датасет, например, со следующими столбцами:
* Base ts - максимальная (последняя) отметка времени из L строк
* Current value - значение в последней из L строк 
* Max value - максимальное значение среди значений из L строк
* ... (фичи, которые можно достать из L строк)
* target1 - значение в первой клетке из H строк
* target2 - значение во второй клетке из H строк
* ... (значения из H строк)

Таким образом, сокращаем количество строк в датасете в STEP раз

Как правильно разбивать датасет на train и test в случае временных рядов:

<p align="center">
<img src="./src/img/03_lect/train_test.png"  style="width: 50%">
</p>

Виды метрик, которые можно смотреть для временных рядов:

<p align="center">
<img src="./src/img/03_lect/errors.png"  style="width: 80%">
</p>

# [Лекция 4] Ранжирование

* [Запись](https://disk.360.yandex.ru/d/-i0quAMsXqNabw/2%20%D0%BA%D1%83%D1%80%D1%81%20%D0%9C%D0%9D%D0%90%D0%94-24/%D0%9C%D0%B0%D1%88%D0%B8%D0%BD%D0%BD%D0%BE%D0%B5%20%D0%BE%D0%B1%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5%202/1%20%D0%BF%D0%BE%D1%82%D0%BE%D0%BA/4.%2027.09.25%20%D0%9C%D0%B0%D1%88%D0%B8%D0%BD%D0%BD%D0%BE%D0%B5%20%D0%BE%D0%B1%D1%83%D1%87%D0%B5%D0%BD%D0%B8%D0%B5%202_%D0%9B%D0%B5%D0%BA%D1%86%D0%B8%D1%8F_1%20%D0%BF%D0%BE%D1%82%D0%BE%D0%BA.mp4)

## Что это? 

Сортировка объектов. Например, веб-страницы в поисковике или товары на маркетплейсе

Формально:
* Есть некоторый набор документов $D$ и запросы $Q$. Для каждого запроса $q$ известны "кандидаты", подходящие под него - $D_q$
* Цель - отсортировать $D_q$ по уменьшению релевантности к $q$

Рекомендательные системы - очень похожая задача

**"Бинарная релевантность"** - лейблы 0/1 для пары запрос-документ. Обычно эти лейблы размечают асессоры, которым дают некоторый запрос и набор документов, для каждого из которых они проставляют 0 или 1 - релевантен этот документ запросу или нет
